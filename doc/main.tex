...
Como se puede ver en la \autoref{fig:red}, se creo una red \textbf{DMZ} que conectase con el router para poder aislar las máquinas 
y reproducir de forma más fiable una red empresarial. La red \textbf{LAN} es la red Wi-Fi del router y la \textbf{WAN} representa internet. 
El router realmente está virtualizado al igual que el resto de contenedores y VM's. Hemos redireccionado el puerto 
\texttt{3478} del router para que permita la conexión con la vpn que montamos. La redireccion del puerto \texttt{80}/\texttt{443} 
hacia la máquina con \texttt{caddy} permite el acceso externo a la infraestructura.

El resto de las máquinas de la red estan aisladas frente accesos no permitidos, 
śolamente acceptan conexiones \texttt{ssh} con certificado 
(se introducieron dentro de las VM nuestros certificados para un acceso más seguro y rápido).
...
\begin{itemize}
  ...
  \item \texttt{grafana}: guardamos un script y el dashboard para cargarlo en la máquina correspondiente. 
\end{itemize}
...
Para comezar se realizó la configuración de un router \textbf{pfsense}~\cite{pfsense} virtual que nos permitiese una buena gestión de las redes y monitoriación. 
Como siguiente paso configuramos un acceso remoto mediante \textbf{TailScale}~\cite{tailscale} como backup, pues este solo permite el acceso de uno de nosotros a la red. 
Montamos entonces otra VPN, \textbf{WireGuard}~\cite{wireguard}, que nos permite el acceso remoto a la red a los 2. 
Ademas el TailScale casi se monta solo por lo que queriamos hacer el WireGuard de 0 como una experiencia más real y segura.

\subsection{Declaración de los recursos}

Para que OpenTofu pueda interactuar con el hipervisor, es necesario configurar el proveedor de Proxmox en el archivo \texttt{main.tf}. 
La autenticación se realiza mediante un \textbf{API Token} para evitar el uso de la contraseña de \texttt{root}.
...
Para automatizar este proceso, utilizamos un script \texttt{tofu.sh} que carga las variables de entorno desde un archivo \texttt{.env}, 
facilitando el despliegue sin exponer credenciales en el código fuente. El uso de una plantilla \textbf{Debian} con \textbf{Cloud-init} es clave: 
nos permite inyectar nuestras claves SSH públicas de forma automática, dejando la máquina lista para que NixOS tome el control.

\subsection{Configuración con nixos-anywhere}

Una vez que OpenTofu ha levantado las máquinas virtuales con Debian, utilizamos \textbf{nixos-anywhere}~\cite{nixos-anywhere} para realizar la transición a NixOS. 
Este proceso es crítico para la naturaleza declarativa del proyecto, ya que permite instalar NixOS en un sistema existente de forma remota.
...
\begin{enumerate}
    ...
    \item \textbf{Particionado con Disko}~\cite{disko}: Definimos el esquema de particiones en un archivo \texttt{disk-config.nix}. 
      \texttt{nixos-anywhere} utiliza esta herramienta para formatear el disco de la VM Debian en caliente.
...
Posteriormente, cualquier cambio en la configuración (servicios, usuarios, Docker) se gestiona con \textbf{Colmena}~\cite{colmena}, 
lo que permite aplicar cambios en segundos sin reiniciar la máquina ni reinstalar el sistema.

\subsection{Gestión de cambios con Colmena}

Una vez realizado el despliegue inicial con \texttt{nixos-anywhere}, la herramienta encargada de gestionar el ciclo de vida de la configuración es \textbf{Colmena}~\cite{colmena}. 
A diferencia de \texttt{nixos-anywhere}, que instala el sistema desde cero, Colmena aplica cambios incrementales sobre máquinas ya en ejecución mediante SSH, 
...
Cada host se define con su dirección IP (\texttt{targetHost}), usuario de despliegue (\texttt{targetUser}) y su configuración NixOS, 
que es exactamente la misma que se usa en \texttt{nixosConfigurations}. Esto garantiza que no existe divergencia entre el sistema instalado y el sistema gestionado.
...
\end{verbatim}
  \item Colmena construye la nueva generación del sistema en la máquina local (o en el propio host si se configura despliegue remoto), 
    la transfiere por SSH y ejecuta \texttt{nixos-rebuild switch}, activando los cambios en caliente.
\end{enumerate}
...
El punto de entrada de toda la configuración NixOS es el archivo \texttt{flake.nix}, que actúa como manifiesto del proyecto: 
declara las dependencias externas (inputs) y expone las configuraciones de cada host (outputs). 
Entre los inputs más relevantes se encuentran \texttt{nixpkgs} (el repositorio de paquetes de NixOS), 
\texttt{agenix}~\cite{agenix} (gestión de secretos), \texttt{disko}~\cite{disko} (particionado declarativo) y \texttt{colmena}~\cite{colmena} (despliegue remoto).

Cada host queda registrado tanto en el output \texttt{nixosConfigurations} (usado por \texttt{nixos-anywhere} en el primer despliegue) 
como en el output \texttt{colmena} (usado para actualizaciones posteriores). 
Esto garantiza que la misma definición de sistema se emplee independientemente del mecanismo de despliegue.

\subsubsection{Módulos compartidos}

Para evitar la duplicación de configuración entre hosts, extraemos toda la lógica reutilizable en módulos bajo \texttt{nixos/modules/}. 
...
\begin{itemize}
  ...
  \item ssh.nix
    ...
    Endurece la configuración del demonio SSH: deshabilita el login con contraseña (\texttt{PasswordAuthentication = false}). 
  ...
  \item fail2ban.nix
    ...
    Habilita el servicio \texttt{fail2ban}~\cite{fail2ban} con una jail para SSH que banea automáticamente direcciones IP tras varios intentos de autenticación fallidos. 
...
\begin{itemize}
  ...
  \item \texttt{disk-config.nix}: define el esquema de particiones mediante \texttt{disko}~\cite{disko}. Usamos una tabla \textbf{GPT} con una partición de arranque BIOS y una partición raíz \texttt{ext4}.
...
mediante la API de \textbf{Cloudflare}~\cite{cloudflare}. Se despliega como un \texttt{systemd.services} periódico que comprueba
la IP pública actual y actualiza el registro DNS correspondiente en caso de cambio, 
usando de nuevo el token de Cloudflare gestionado por \texttt{agenix}~\cite{agenix}.
...
\begin{itemize}
  \item \textbf{node\_exporter.nix}: habilitado en \textit{todos} los hosts, expone métricas del sistema (CPU, memoria, disco, red)
    en el puerto \texttt{9100} mediante \texttt{services.prometheus.exporters.node}~\cite{prometheus}.
  \item \textbf{prometheus.nix}: desplegado únicamente en \texttt{monitoring}, configura \texttt{services.prometheus}~\cite{prometheus} con un \texttt{scrapeConfig} que apunta 
    a los exporters de cada worker y del propio host Caddy. El intervalo de scraping y las reglas de retención se declaran en este módulo.
  \item \textbf{grafana.nix}: también exclusivo de \texttt{monitoring}, levanta \texttt{services.grafana}~\cite{grafana} con Prometheus como datasource provisionado automáticamente.
    Los dashboards se importan como JSON y se exponen al exterior a través del proxy inverso Caddy.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Seguridad}: Gracias a \texttt{agenix}~\cite{agenix}, los secretos (tokens de Cloudflare, contraseñas de Docker) viajan cifrados
...
\begin{itemize}
    ...
    \item \textbf{Calidad del Provider}: El proveedor de Proxmox~\cite{proxmox} para OpenTofu~\cite{opentofu}/Terraform~\cite{terraform} presenta limitaciones 
      ...
      esperas para que las VM's vuelvan a estar disponibles, esto es acendutado por el hardware que empleamos (los discos duros y controladora principalmente).
    \item \textbf{Automatización incompleta}: No hemos logrado una ``automatización de un solo clic'' (\textit{full automation});
      ...
      además es necesario realizar la configuración (paso) de clave de ssh para agenix de forma manual (por ahora).
\end{itemize}
